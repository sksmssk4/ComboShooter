...
{		
	...
	//타이머
	D3DXCreateTextureFromFileEx(d3ddev,L"dz.png",D3DX_DEFAULT,D3DX_DEFAULT,D3DX_DEFAULT,NULL,D3DFMT_A8R8G8B8,D3DPOOL_MANAGED,
		D3DX_DEFAULT,D3DX_DEFAULT,D3DCOLOR_XRGB(255, 0, 255),NULL,NULL,&sprite_dz);
	D3DXCreateTextureFromFileEx(d3ddev,L"dz2.png",72,59, D3DX_DEFAULT,NULL,D3DFMT_A8R8G8B8,D3DPOOL_MANAGED,
...
void Game::render_frame(void)
{
	if (ingame == true)
	{
		...
		if (summonitem2.Appear == true)
		{
			...
			{
				if (bottle[i].breaking == true && effecting == false)
				{
					RECT part1;
					SetRect(&part1, 0, 0, 64 * (int)(bottle[i].scale), 64 * (int)(bottle[i].scale));
					D3DXVECTOR3 center1(0.0f, 0.0f, 0.0f);
				...
				////bottle	
				RECT rc;
				SetRect(&rc, 0, 0, 64 * (int)(bottle[i].scale), 64 * (int)(bottle[i].scale));
...
void Game::do_game_logic(void)
{
	...
	if (score.GetNum() == 3)
		...
		dNum = 3;
	else if (score.GetNum() == 45)
		dNum = 2;
	else if (score.GetNum() == 55)
		dNum = 1;
	else if (score.GetNum() == 65)
		dNum = 0;
